...
Class Action {
	...
	function save_user(){
		...
		foreach($_POST as $k => $v){
            if (is_null($v))
                exit();
			if(!in_array($k, array('id','cpass','password','child_group')) && !is_numeric($k)){
				if(empty($data)){
					$data .= " $k='$v' ";
				}else{
					$data .= ", $k='$v' ";
        ...
        if(isset($group_id)){
            ...
            if (is_array($child_group))
                $data .= ", child_group='".implode(',',$child_group)."' ";
            else if (isset($child_group))
                $data .= ", child_group='".$child_group."' ";
        }
//		if(!empty($password)){
